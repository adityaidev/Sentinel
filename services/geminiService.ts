...
export const runReporterAgent = async (swot: SWOT, rawData: string, company: string): Promise<{ report: string, log: LogEntry }> => {
  ...
  try {
    ...
    return {
        report: response.text || "# Error Generating Report",
        log: {
            timestamp: new Date().toISOString(),
            agent: AgentRole.REPORTER,
            message: `Compiled final executive report for ${company}`,
            type: 'success',
            latencyMs: endTime - startTime,
            tokenUsage: (response.usageMetadata?.totalTokenCount || 0),
...
// --- 6. Social Agent ---
export const runSocialAgent = async (report: string, company: string): Promise<{ post: string, log: LogEntry }> => {
    const startTime = performance.now();
    const ai = new GoogleGenAI({ apiKey: API_KEY });
    ...
    try {
        const response = await ai.models.generateContent({
            ...
            config: {
                systemInstruction: PROMPTS.SOCIAL_SYSTEM
            }
        });
        ...
        return {
            ...
            log: {
                ...
                message: `Generated viral social media post draft`,
                type: 'success',
                latencyMs: endTime - startTime,
                tokenUsage: (response.usageMetadata?.totalTokenCount || 0),
                cost: calculateCost(MODELS.SOCIAL, response.usageMetadata)
            }
        };
    } catch (error) {
...
// --- 7. Chat Utility for Report View ---
export const createChatSession = (context: string) => {
  const ai = new GoogleGenAI({ apiKey: API_KEY });
  return ai.chats.create({
    model: MODELS.ANALYST,
    config: {
      systemInstruction: `You are an expert Strategy Consultant having a conversation about a specific competitive analysis report.
      
      CONTEXT OF ANALYSIS:
      ${context}
      
      Answer questions based on this context. Be concise, professional, and data-driven.`,
    }
  });
};
